...
import { isClosed, lifecycle, normalizedStatus, scheduleLabel, scheduleShortLabel, workflowFor, type Application, type StageKey, type WorkflowStage } from '../model'
...
type ProgressStep = { key?: StageKey; label: string; date: string; state: string; stage?: WorkflowStage }
...
export function ApplicationProgress({ app, compact = false, selectedStage, onSelectStage, onInsertAfter }: ProgressProps) {
  ...
  const steps: ProgressStep[] = [
    ...fields.map(stage => ({ key: stage.id, label: stage.label, date: scheduleShortLabel(stage), state: stage.status, stage })),
  ...
  return <ol className={`job-progress ${compact ? 'compact' : ''} ${onInsertAfter ? 'editable' : ''}`} style={{ gridTemplateColumns: `repeat(${steps.length}, minmax(${compact ? '46px' : '100px'}, 1fr))` }} aria-label="招聘流程进度">
    {steps.map(step => {
      ...
      const content = <><span className="job-step-dot">{failed || cancelled ? <X size={10} /> : complete ? <Check size={11} /> : current ? <i /> : null}</span><span className="job-step-label">{step.label}</span><time title={step.stage ? scheduleLabel(step.stage) : undefined}>{step.date || '—'}</time></>
      const key = step.key
      return <li key={key || step.label} className={`${complete ? 'complete' : ''} ${current ? 'current' : ''} ${failed ? 'failed' : ''} ${cancelled ? 'cancelled' : ''} ${key === selectedStage ? 'selected' : ''}`} title={`${step.label}：${current ? '当前阶段' : step.state}${step.stage && scheduleLabel(step.stage) ? ` · ${scheduleLabel(step.stage)}` : ''}`}>
