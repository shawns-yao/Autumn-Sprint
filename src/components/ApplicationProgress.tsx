...
export function ApplicationProgress({ app, compact = false, selectedStage, onSelectStage, onInsertAfter }: ProgressProps) {
  ...
  return <ol className={`job-progress ${compact ? 'compact' : ''} ${onInsertAfter ? 'editable' : ''}`} style={{ gridTemplateColumns: `repeat(${steps.length}, minmax(${compact ? '46px' : '100px'}, 1fr))` }} aria-label="招聘流程进度">
    {steps.map(step => {
      ...
      const key = step.key
      return <li key={key || step.label} className={`${complete ? 'complete' : ''} ${current ? 'current' : ''} ${failed ? 'failed' : ''} ${key === selectedStage ? 'selected' : ''}`} title={`${step.label}：${current ? '当前阶段' : step.state}${step.date ? ` · ${step.date}` : ''}`}>
        ...
        {onInsertAfter && step.label !== 'Offer' && <button type="button" className="job-step-insert" aria-label={`在${key ? step.label : '投递'}后添加阶段`} title={`在${key ? step.label : '投递'}后添加阶段`} onClick={() => onInsertAfter(key || '')}><Plus size={13} aria-hidden="true" /></button>}
      </li>
    })}
  </ol>
}
