...
import { Heading, Tooltip } from './Shared'
import './overview.css'
...
export default function Overview({ apps, onOpen, onView, action }: Props) {
  ...
  const stageNames = [...new Set(active.flatMap(app => workflowFor(app).map(stage => stage.label)))]
  ...
  return <section className="home-dashboard">
    ...
    <div className="home-grid">
      ...
      <section className="home-section">
        <header className="home-section-heading"><Filter /><div><h2>投递漏斗</h2><p>当前未终止岗位 · 共 {funnel[0]?.count || 0} 个</p></div><select aria-label="流程统计公司" value={company} onChange={event => setCompany(event.target.value)}><option value="">全部岗位</option>{[...new Set(apps.map(app => app.company))].map(name => <option key={name} value={name}>{name}</option>)}</select></header>
        <div className="home-funnel" aria-label="四阶段招聘记录漏斗">
          {funnel.map((item, index) => <div className="home-funnel-row" key={item.label}>
            ...
            <div className="home-funnel-stat"><strong>{funnelTotal ? `${Math.round(item.count / funnelTotal * 100)}%` : '—'}</strong><span>占当前投递</span></div>
          </div>)}
        </div>
      </section>
