...
import NoteEditor, { NoteRichContent } from './NoteEditor'
...
const noteExcerpt = (body: string) => body.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
...
export default function Notes({ apps, onOpenJob }: Props) {
  ...
  return <section className="notes-page">
    <Heading className="notes-page-header" title="笔记" meta="沉淀可复用的面试内容、项目话术与经验总结。" action={<div className="notes-header-actions"><SearchField className="notes-search" label="搜索笔记" value={query} disabled={Boolean(editor)} onChange={event => { setQuery(event.target.value); setDeleteId(null) }} placeholder="搜索笔记标题、内容或标签…" /><Button variant="primary" icon={Plus} onClick={startNew} disabled={loading || busy || Boolean(loaded.error) || Boolean(editor)}>新建笔记</Button></div>} />
    ...
    <div className="notes-categories" role="group" aria-label="笔记分类"><button disabled={Boolean(editor)} aria-pressed={!category} onClick={() => { setCategory(''); setDeleteId(null) }}>全部</button>{categories.map(value => <button disabled={Boolean(editor)} key={value} aria-pressed={category === value} onClick={() => { setCategory(value); setDeleteId(null) }}>{value}</button>)}</div>
    ...
    <div className="notes-workspace">
      <aside className="notes-list-pane"><header><h2>我的笔记（{filtered.length}）</h2><select aria-label="笔记排序" value={sort} onChange={event => setSort(event.target.value)}><option value="updated">按更新时间</option><option value="oldest">最早更新</option><option value="title">按标题</option></select></header>
        <div className="notes-list">{filtered.map(note => {
          ...
          return <button className={`notes-list-row ${selected?.id === note.id ? 'selected' : ''}`} key={note.id} aria-pressed={selected?.id === note.id} disabled={Boolean(editor && editor.id !== note.id)} onClick={() => { setSelectedId(note.id); setDeleteId(null); setActionError('') }}>
            <span className={`notes-symbol notes-color-${color}`}><Icon size={25} strokeWidth={1.8} /></span><span className="notes-list-content"><span className="notes-row-top"><strong>{note.title}</strong><span className={`notes-category-badge notes-color-${color}`}>{note.category}</span><time dateTime={note.updatedAt}>{noteDate(note.updatedAt)}</time></span><span className="notes-excerpt">{noteExcerpt(note.body).slice(0, 140) || '暂无正文'}</span></span>
          </button>
      ...
      <article className={`notes-reader ${editor ? 'is-editing' : ''}`}>{editor ? <NoteEditor key={editor.id} note={editor} onSave={save} onClose={() => setEditor(null)} /> : selected ? <>
        ...
        <div className="notes-rich-reading"><NoteRichContent key={selected.id} value={selected.body} /></div>
